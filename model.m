...
N=1000;
p=1-N/M;
timesteps = M; % This is also the number of nodes.
...
for t = 1:timesteps-1
%    if rand()<p && t>10
%        [x, y] = ind2sub(size(A), datasample(find(A==0),1) );
%        A(x,y)=1;
%        A(y,x)=1;
%    end
    ...
    deg_weight = arrayfun(@(x) (x/max(deg))^2,deg);
    ...
    if rand()<p
        nbors=find(A(:,n));
        nbors=nbors(find(nbors~=A(size(A,1),n)));
        
        nbor_lin_idx=randsample(nbors,1);
        [nbor_x n_copy]= ind2sub(size(A),nbor_lin_idx);
        A(size(A,1),nbor_x) = 1;  
        A(nbor_x,size(A,2)) = 1;
    end
end
